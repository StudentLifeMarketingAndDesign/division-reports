...
class MigrateBlocksToElementalTask extends BuildTask{
	...
	function run($request){
		...
		$elementAreaNames = array(
			'BeforeContent' => 'ElementalArea',
			'BeforeContentConstrained' => 'ElementalArea',
			'AfterContent' => 'ElementalArea',
			'AfterContentConstrained' => 'ElementalArea',
			'SidebarArea' => 'ElementalArea'
		...
		foreach($elements as $element){
			...
			foreach($results as $result){
				...
				if($page){
                    ...
                    $textBlockResult = $elementQuery->setFrom('ContentBlock')->setSelect('*')->setWhere("\"ID\" = '".$elementID."'")->execute()->first();



					$element->ParentID = $page->obj('ElementalArea')->ID;
					$element->Sort = $result['Sort'];
					$element->HTML = $elementResult['Content'];
                    if(isset($textBlockResult['ImageID'])){
                        $element->ImageID = $textBlockResult['ImageID'];
                    }

                    if(isset($textBlockResult['ExternalLink'])){
                        $element->ExternalLink = $textBlockResult['ExternalLink'];
                    }

                    if(isset($textBlockResult['LinkedPageID'])){
                        $element->LinkedPageID = $textBlockResult['LinkedPageID'];
                    }
